  ...
  <table className="min-w-[720px]">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>Consistency</td>
        <td>
          In Concurrent Mode, ref values are not guaranteed to stay stable
          during a render
        </td>
      </tr>
    </tbody>
...
The second sentence is about consistency in Concurrent Mode: when rendering is interrupted and resumed, `ref.current` can change in the middle. Because it is mutable data outside React’s control, each render attempt can read a different value.
...
The most serious problem appears in Concurrent Mode. React can split rendering into time slices. It can render the upper part of the tree, yield, and later continue with the lower part.
...
The docs explicitly allow an exception: “except for initialization.” This does not mean Concurrent Mode is turned off during initialization. It still works the same way. The exception is allowed because of **idempotency**.
...
The React team even left the comment `// Read: safe because lazy init:`. It is not “safe because concurrency is off,” but “safe because retries still produce the same result in concurrency.”
...
Even with direct mutation, JavaScript does not block it and React runtime does not throw. React just fails to detect the change, so the UI does not update.
...
<div className="my-4 overflow-x-auto">
  <table className="min-w-[720px]">
    ...
    <tbody>
      ...
      <tr>
        <td>DEV warning</td>
        <td>StrictMode double render tries to expose it</td>
        <td>Warning for reading refs during render</td>
      </tr>
      <tr>
        <td>If you violate it</td>
        <td>React misses the change -> UI does not update</td>
...
Both break React’s core assumption: “render is a pure function.” Since JavaScript cannot block these patterns at language level, React relies on DEV warnings and documentation. Keeping that assumption is the developer’s responsibility.
