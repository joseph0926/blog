...
Another part of this was isolation. When testing React Query, I ended up preferring a fresh `QueryClient` per test. With Zustand, I also found that leaving shared state behind between tests makes failures harder to trust. That is why I went back to [TkDodo's React Query post](https://tkdodo.eu/blog/testing-react-query) and the [Zustand testing guide](https://zustand.docs.pmnd.rs/guides/testing). Keep as much real logic as possible, but do not let state leak across tests.
