...
여기서 말하는 `import`는 **정적 import 선언문**입니다. 정적 import는 파일 최상단에만 올 수 있어서, 코드를 실행하지 않아도 "이 모듈이 어떤 모듈을 가져오는지"가 확정됩니다. 반면 `import()`는 표현식이므로 조건문/함수 안에서 사용할 수 있습니다.
...
`export`는 모듈 top-level에서만 선언되고 조건문 안에 들어갈 수 없어서, "이 모듈이 무엇을 내보내는지"가 확정됩니다.
