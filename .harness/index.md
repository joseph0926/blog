...
이 문서들이 아직 현재 저장소를 설명하는지 검사한다. `baseline_revision` 이후 커밋 수, 문서가 가리키는 경로 실존, frontmatter를 본다. 파일 참조가 깨지면 exit 1이다.

**작업 전에 한 번 돌리는 것을 권한다.** 그러지 않으면 낡은 문서를 현재 사실로 읽게 된다.
